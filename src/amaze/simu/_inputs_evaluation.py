...
InputDetails = Tuple[np.ndarray,              # Walls
                     Action,                  # Correct action
                     Optional[Direction],     # Previous direction
                     Optional[OrientedSign]]  # Sign and orientation
...
def __draw_arrow(painter, color, rotation, size, height):
    ...
    painter.scale(.9 * size, .9 * height * size)
    painter.translate(-.5, -.5)
    painter.fillPath(ARROW_PATH, color)
    painter.restore()


def __draw_action(painter: QPainter, correct_action, action,
                  size: float):
    def a_to_dir(a):
    ...
    if correct_action != action:
        __draw_arrow(painter, Qt.blue, correct_direction.value, size, .5)
...
def __draw_discrete_input(
        details: InputDetails, _, action: Action = None):
    ...
    if oriented_sign is not None:
        ...
        __draw_arrow(painter, QColor.fromHsvF(0, 0, sign.value),
                     direction.value,
                     1 - w, 1)
...
def __draw_image_input(details: InputDetails,
                       visual: np.ndarray,
                       action: Action):
    ...
    w, h = visual.shape
    img = QImage((visual * 255).astype(np.uint8)
                               .repeat(scale, axis=0)
                               .repeat(scale, axis=1).data,
                 scale * w, scale * h, scale * w, QImage.Format_Grayscale8)
...
def __draw_inputs(path: Path,
    ...
    margin = 4
    if summary_file:
        big_image = QImage(cols * (size + margin),
                           rows * (size + margin),
    ...
    for i, (obs, visual, action) in \
            enumerate(itertools.zip_longest(inputs, visuals, outputs or [])):
        ...
        if summary_file:
            x, y = i % cols, i // cols
            painter.drawImage(QPointF(x * (size + margin) + .5 * margin,
                                      y * (size + margin) + .5 * margin),
                              img)
...
def inputs_evaluation(path: Path,
    ...
    for walls, correct_action, prev_dir, oriented_sign in inputs:
        ...
        sign, sign_type, sign_direction = (oriented_sign or
                                           (None, None, None))
        df.loc[len(df)] = [
            "".join(str(int(_w)) for _w in walls), cell_type,
            sign.to_string() if sign else NO_VALUE,
            _maybe_enum(sign_type), _maybe_enum(sign_direction),
    ...
    if draw_inputs:
        __draw_inputs(path.joinpath("inputs.png"), inputs, visuals,
                      outputs=None,
    ...
    if draw_individual_files or draw_summary_file:
        __draw_inputs(path.joinpath("inputs_recognition.png"),
                      inputs, visuals, outputs,
